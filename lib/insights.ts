...
export async function getUserInsights(userId: string, startDate?: Date, endDate?: Date): Promise<InsightsStats> {
  ...
  const cacheKey = `insights:${userId}:${startDate?.getTime() || "default"}:${endDate?.getTime() || "now"}`

  return cached(
    cacheKey,
    async () => {
      console.log("[v0] Computing insights for user:", userId)
...
export async function getUserDetailedAnalytics(
  ...
  const cacheKey = `detailed:${userId}:${startDate?.getTime() || "default"}:${endDate?.getTime() || "now"}`

  return cached(
    cacheKey,
    async () => {
      console.log("[v0] Computing detailed analytics for user:", userId)
      ...
      const [geographic, devices, browsers, referrers] = await Promise.all([
    ...
    90, // Cache for 90 seconds (detailed analytics changes less frequently)
  )
}
